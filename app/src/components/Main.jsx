...
export default function Main({ network, voteAccount }) {
  ...
  useEffect(() => {
    ...
    async function getVotes() {
      ...
      try {
        ...
        setVotes({
          stephen: parseInt(account.stephen.toString()),
          lebron: parseInt(account.lebron.toString()),
        });
      } catch (error) {
  ...
  async function handleVote(side) {
    ...
    try {
      const tx =
        side === "stephen"
          ? await program.rpc.voteStephen({
              accounts: {
                voteAccount: voteAccount.publicKey,
              },
            })
          : await program.rpc.voteLebron({
              accounts: {
                voteAccount: voteAccount.publicKey,
              },
            });
      ...
      <Box flex="1 0 auto">
        ...
        <Container>
          <Grid container spacing={3}>
            ...
            <Grid item xs={6}>
              <VoteOption side="stephen" handleVote={handleVote} />
            </Grid>
            <Grid item xs={6}>
              <VoteOption side="lebron" handleVote={handleVote} />
            </Grid>
            <Grid item xs={12}>
