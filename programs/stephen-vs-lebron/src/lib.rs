...
declare_id!("GrUhBnyVvJzUUYRMz83pEcewmPdf8BFKdFuXie8mRkkn");
...
pub mod stephen_vs_lebron {
    ...
    pub fn initialize(ctx: Context<Initialize>) -> ProgramResult {
        ...
        vote_account.stephen = 0;
        vote_account.lebron = 0;
        Ok(())
    }
    /// All account validation logic is handled below at the #[account(...)] macros, letting us just focus on the business logic
    pub fn vote_stephen(ctx: Context<Vote>) -> ProgramResult {
        let vote_account = &mut ctx.accounts.vote_account;
        vote_account.stephen += 1;
        Ok(())
    }
    pub fn vote_lebron(ctx: Context<Vote>) -> ProgramResult {
        let vote_account = &mut ctx.accounts.vote_account;
        vote_account.lebron += 1;
        Ok(())
    }
}
...
/// We define a struct with two public properties: stephen and lebron
